...
- **メモリ管理機能**: 長時間稼働時のメモリ使用量を監視・制御し、システムリソースを効率的に利用します。
...
- アイドル検出機能により、ユーザーが操作していない時は自動的にポーリング頻度を削減します。

### メモリ管理

長時間稼働時のシステムリソースを効率的に管理するための包括的なメモリ管理機能を搭載しています。

- **リアルタイム監視**: 30秒ごとにメモリ使用量を監視し、現在の使用量（MB単位およびパーセント）を設定画面で確認できます。
- **設定可能な上限値**: 
  - メモリ使用量の上限をMB単位（512MB〜8192MB）で設定可能
  - 物理メモリに対する使用割合の上限をパーセント（20%〜80%）で設定可能
- **自動的な負荷軽減**:
  - メモリ使用量が設定値を超えると自動的に低メモリモードに移行
  - 低メモリモードでは処理レベルを下げ、ポーリング間隔を延長
  - メモリ使用量が設定値の半分以下になると通常モードに自動復帰
- **メモリ最適化技術**:
  - LRUキャッシュによる効率的なメモリ管理
  - ストリーミング処理による大容量ファイルの読み込み
  - 適応的な処理レベル制御（disabled/minimal/normal/full）
  - 強制的なバッファフラッシュとキャッシュクリア機能
- **CPU使用率の最適化**:
  - UI要素の再帰探索に深度制限（最大10階層）を設定
  - 処理レベルに応じた浄化処理頻度の動的調整
  - アイドル時のポーリング頻度自動削減
